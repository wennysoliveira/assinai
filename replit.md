...
- **subscriptions** — id, customer_id, plan, periodicity (monthly/annual), amount, status (active/cancelled/overdue), next_billing_date
...
- **subscriptions** — CRUD for subscription management (delete = cancel)
...
- **Subscriptions** (/subscriptions) — CRUD with customer selection and status filter
- **Invoices** (/invoices) — List with PIX generation and WhatsApp reminder actions
- **Settings** (/settings) — API configuration overview and webhook URLs
